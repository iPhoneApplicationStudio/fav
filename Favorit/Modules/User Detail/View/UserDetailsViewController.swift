...
class UserDetailsViewController: UIViewController {
    ...
    var follow: UserFollowedResponse?
    
    var isMyProfile: Bool {
        return userSessionService.loggedInUserID == userId
    }
...
extension UserDetailsViewController {
    ...
    func getUser() {
        ...
        userDetailService.getUserDetails(UserDetailRequest(userID: userId)) { result in
            switch result {
            case .success(let profileUser):
                self.setupUser(user: profileUser)
...
extension UserDetailsViewController {
    ...
    func setupUser(user: User) {
        ...
        let followerCount = user.followers?.integer ?? 0
        let followingCount = user.following?.integer ?? 0
        let favoritCount = 0 //user.favoritsCount ?? 0
        let bookmarkCount = 0 //self.user?.bookmarksCount ?? 0
        ...
        self.favoritCountLabel.textColor = Colors.accentColor
...
//        }
    ...
    func followerUser() {
        userFollowService.follow(userID: userId) { result in
            switch result {
            case .success(let response):
                self.followButton.isSelected = true
                self.follow = response
//                self.checkIfFollowingUser()
    ...
    func unfollowUser() {
        followButton.isSelected = false
        guard let unfollow = follow?._id else {
            return
        }
        userFollowService.unFollow(userID: unfollow) { result in
            switch result {
            case .success:
                self.follow = nil
            case .failure(let failure):
                self.showError(message: "There was an error \(failure.localizedDescription)")
            }
        }
    }
}
